		...
		<section class="wrapper">
			<div class="row">
				<div class="col-lg-12">
					...
					<ul class="breadcrumb">
						...
						<li><a href="<?=base_url().'admin/'.$link?>"> <?=$prev_page?></a></li>
            ...
            <?php if($this->session->flashdata('breakmessage')) { ?> 
                <div class="alert alert-block alert-danger fade in" id="alert">
                    <?= $this->session->flashdata('breakmessage'); ?>
                </div> 
            <?php } ?>
				...
				<div class="col-lg-12">
					<section class="panel">
						...
						<div class="panel-body">
							<form method="post" action="<?=base_url().'admin/rule/fn_update_point_rule'?>" enctype="multipart/form-data">
								<div class="row">
									<div class="col-lg-6">
										<div class="form-group">
											<label for="title">Title</label>
											<input type="text" class="form-control" name="title" id="title" placeholder="Title" value="<?= $title ?>" required>
                                        </div>
                                        <div class="form-group">
											<label for="title_id">Title Indonesia</label>
											<input type="text" class="form-control" name="title_id" id="title_id" placeholder="Title Indonesia" value="<?= $title_id ?>" required>
                                        </div>
                                        <div class="form-group">
                                            <label for="description">Description</label>
                                            <textarea rows="4" cols="50" id="description" class="form-control" name="description"  required> <?= $description ?></textarea>
                                        </div>
                                        <div class="form-group">
											<label for="description_id">Description Indonesia</label>
                                            <textarea rows="4" cols="50" id="description_id" class="form-control" name="description_id" required><?= $description_id ?></textarea>
										</div>
										<div class="form-group">
											<label for="point">Point</label>
											<input type="number" min="0"  class="form-control"  id="point" name="point" placeholder="Point" value="<?= $point ?>" required>
										</div>
                                    </div>
                                    <div class="col-lg-6">
                                        <div class="form-group">
                                            ...
                                            <div class="radio">
                                                <label>
                                                    <input type="radio" name="active" value="true" <?php echo ($active == true)?'checked':'' ?> required>
                                                    Yes
                                                </label>
                                            </div>
                                            <div class="radio">
                                                <label>
                                                    <input type="radio" name="active" value="false" <?php echo ($active == false)?'checked':'' ?> required>
                                                    No
                                                </label>
                                            </div>
                                        </div>
                                            ...
                                            <select class="form-control m-bot15" name="type" required>
                                                <?php
                                                    foreach ($objType as $item) {
                                                    	?>
                                                    <option <?= $type == $item->type ? "selected" : "" ?> value="<?php echo $item->idtype ?>">
                                                        <?php echo $item->type ?>
                                                    </option>
                                                <?php } ?>
                                            </select>
                                        ...
                                        <div class="form-group">
                                            <input type="checkbox" style="width: 2vw; height: 2.5vh;" id="disable_date" name="disable_date" <?= !$validuntil?"checked":"" ?>>
                                            <label>Valid all the time</label>
                                        </div>
                                        <div class="form-group limit_rule" <?= $limit==null ? 'style="display:none"': '' ?>>
                                            <label for="limit">Limit</label>
											<input type="number" min="0" value="<?= $limit!=null ? $limit : 1 ?>" class="form-control" id="limit" name="limit" placeholder="Rule limit" required>
                                        </div>
                                        <div class="form-group">
                                            <input type="checkbox" <?= $limit!=null ? 'checked': '' ?> style="width: 2vw; height: 2.5vh;" id="limit_rule" name="is_limit_rule" >
                                        ...
                                        <input type="hidden" class="form-control" name="idtenant" id="idtenant"  value="<?= implode(",",$tenant) ?>">
...
<script type="text/javascript">
    ...
    var disable_date = document.querySelectorAll('input[name="disable_date"]');
    ...
    if(validfromdate.value == ""  && validuntildate.value == "" ){
        Array.prototype.forEach.call(disable_date, function(el) {
            el.checked = true;
        ...
        // var prevValidUntil = validuntil.value;
        // var prevValidFrom = validfrom.value;
        Array.prototype.forEach.call(disable_date, function(el) {
            el.checked = false;
        });
    }
    ...
    $("#disable_date").click(function() {
        ...
        if(cb.length > 0){
            ...
            // validuntil.value = prevValidUntil?prevValidUntil:null;
            // validfrom.value = prevValidFrom?prevValidFrom:null;
    ...
    var idtenant = document.getElementById('idtenant').value;
    var arrayidTenant = [];
    var arrayTenant = [];
    if(idtenant.length > 0){
        tempidtenant = [];
        tempidtenant.push(idtenant.split(','));
        tempidtenant.forEach(function(item){
            for(var i=0;i<item.length;i++){
                $.ajax({
                    type:"get",
                    url: "<?php echo base_api(); ?>Tenant/?action=retrieve_get&idtenant="+item[i]+"",
                    success: function (data) {
                        addtenant(parseInt(data.detail[0].idtenant),data.detail[0].name,data.detail[0].catname)
                    }
                })
            }
        })
